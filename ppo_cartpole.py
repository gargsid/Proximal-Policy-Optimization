...
logs_file = os.path.join('models', 'ppo_cartpole.txt')
only_print = False 
    ...
    while episodes_completed < max_episodes:
        ...
        for k in range(K):
            ...
            for i in range(N * T // batch_size):
                ...
                # print('ploss:', avg_policy_loss)
                # print('entropy:', avg_entropy)
                # print('vf:', avg_critic_loss)
                # print('kl:', avg_kl_div)
...
# print(graphs['x_ticks_episodes'])

# x_ticks_episodes = np.array(graphs['x_ticks_episodes'])
# x_ticks_episodes = np.mean(x_ticks_episodes, axis=0)

# x = len(x_ticks_episodes)
...
for k, v in graphs.items():
    ...
    path = os.path.join('images', 'cartpole', '{}.jpg'.format(k))
    plt.savefig(path)
    plt.close()
